...
export const selectAllQuestionType = defineType({
  name: "selectAllQuestion",
  ...
  fields: [
    ...
    defineField({
      ...
      validation: (Rule) => [
        Rule.required().min(3).error("Add at least three options"),
        Rule.max(8).warning("Keep options to 8 or fewer for readability"),
        Rule.custom((options) => {
          ...
          if (correctCount === list.length) {
            return "Not all options can be correct";
          }
          return true;
        }),
      ],
